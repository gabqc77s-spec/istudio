...
const ConfigPanel = ({ config, setConfig }) => {
  ...
  const handleContentChange = (e) => {
    setConfig({ ...config, content: { ...config.content, [e.target.name]: e.target.value } });
  };

  const handleBackgroundChange = (key, value) => {
    setConfig({ ...config, background: { ...config.background, [key]: value } });
  }
  ...
  return (
    <div style={panelStyles}>
      ...
      <div style={sectionStyles}>
        <h4 style={{ fontWeight: 'bold', marginBottom: '12px' }}>Content</h4>
        <label style={labelStyles}>Logo URL</label>
        <input type="text" name="logoUrl" value={config.content.logoUrl} onChange={handleContentChange} style={inputStyles} placeholder="https://..."/>
        <label style={labelStyles}>Main Title</label>
        <input type="text" name="title" value={config.content.title} onChange={handleContentChange} style={inputStyles} />
        <label style={labelStyles}>Subtitle</label>
        <input type="text" name="subtitle" value={config.content.subtitle} onChange={handleContentChange} style={inputStyles} />

        <label style={labelStyles}>Vertical Align</label>
        <select name="justifyContent" value={config.content.justifyContent} onChange={handleContentChange} style={selectStyles}>
        ...
        <select name="alignItems" value={config.content.alignItems} onChange={handleContentChange} style={selectStyles}>
